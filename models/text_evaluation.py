...
def load_offensive_dict_with_tokens(json_path="offensive_words.json"):
    ...
    for w in words:
        ...
        w_tokens = cached_tokenize(w_norm)
        results.append({
            "original": w,
            "norm": w_norm,
            "tokens": w_tokens
...
def evaluate_text(
    ...
    # 2) トークン分割（従来ロジックとの併用のため残す）
    input_tokens = tokenize_and_lemmatize(input_norm)

    found_offensive = []
    threshold = 85  # 部分一致の閾値

    for item in offensive_list:
        dict_original = item["original"]
        dict_norm = item["norm"]       # 正規化済み辞書ワード
        dict_tokens = item["tokens"]   # トークン分割結果

        # --- (A) “入力全体 vs. 辞書単語” ファジーマッチ ---
        score = fuzz.partial_ratio(dict_norm, input_norm)
        if score >= threshold:
        ...
        # --- (B) 従来の「入力トークン vs 辞書トークン」ロジック ---
        matched_all = True
        for dict_token in dict_tokens:
            any_match = any(token_match(dict_token, inp) for inp in input_tokens)
            if not any_match:
                matched_all = False
                break
    ...
    # 暴力・ハラスメント・脅迫 などを substring/fuzzy で判定
