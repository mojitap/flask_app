...
# CSVファイルを作成（dataフォルダに保存）
csv_file_path = 'data/court_cases.csv'
...
# ページ番号を自動で増やす方法
current_page = 1
while current_page <= 65:  # ページ数を把握しているなら、65など適切な最大値を設定
    url = f'https://www.courts.go.jp/app/hanrei_jp/list1?page={current_page}&sort=2&filter%5Btext1%5D=%E5%90%8D%E8%AA%89%E6%AF%80%E6%90%8D'
    url = scrape_page(url)
    current_page += 1
